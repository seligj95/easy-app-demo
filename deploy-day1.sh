...
echo "Click \"Customize Source Code →\" in the app, or run this command:"
echo ""
echo "  git clone https://github.com/seligj95/easy-app-demo my-agent-ui \\"
echo "    && cd my-agent-ui \\"
echo "    && mkdir -p .azure/dev \\"
echo "    && cat > .azure/dev/.env << 'EOF'"
echo "  AZURE_APP_NAME=${APP_NAME}"
echo "  AZURE_RESOURCE_GROUP=${RESOURCE_GROUP}"
echo "  AZURE_LOCATION=${LOCATION}"
echo "  AZURE_AI_PROJECT_ENDPOINT=${FOUNDRY_ENDPOINT}"
echo "  AZURE_AI_AGENT_NAME=${AGENT_NAME}"
echo "  AZURE_SKU=${SKU}"
echo "  AZURE_ENTRA_CLIENT_ID=${ENTRA_CLIENT_ID}"
echo "  EOF"
echo ""
echo "Then make your changes and deploy with: azd auth login && azd up"
